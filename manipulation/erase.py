...
TARGET_CATEGORY = 6   # to be edited sketch category
TARGET_SAMPLE_IDX = 18   # to be edited sketch idx
DELETE_INDICES = [3]
...
def shift_delete_multiple_stroke_embeddings_single_sample(
    stroke_emb: torch.Tensor,
    stroke_nums: int,
    delete_indices: list[int]) -> tuple[torch.Tensor, int]:

    assert stroke_emb.shape[0] == 1, "Only supports batch_size = 1"
    B, N, D = stroke_emb.shape
    assert stroke_nums <= N, f"The number of valid strokes {stroke_nums} should not exceed the total length {N}"
    delete_indices = sorted(set(delete_indices))  
    assert all(0 <= idx < stroke_nums for idx in delete_indices), f"The deletion index must be less than stroke_nums"

    stroke_emb = stroke_emb.clone()

    keep_indices = [i for i in range(stroke_nums) if i not in delete_indices]
    num_to_keep = len(keep_indices)
    num_to_delete = len(delete_indices)

    for new_idx, old_idx in enumerate(keep_indices):
        stroke_emb[0, new_idx] = stroke_emb[0, old_idx]

    if num_to_keep > 0:
        last_valid_emb = stroke_emb[0, num_to_keep - 1]
    else:
        last_valid_emb = torch.zeros((D,), dtype=stroke_emb.dtype, device=stroke_emb.device)

    for i in range(num_to_keep, stroke_nums):
        stroke_emb[0, i] = last_valid_emb

    return stroke_emb, stroke_nums - num_to_delete
    ...
    def eval(self, dataloader, category, target_index=None):
        ...
        for batch in dataloader:
            ...
            with torch.no_grad():
                ...
                stroke_emb, new_stroke_num = shift_delete_multiple_stroke_embeddings_single_sample(
                    stroke_emb,
                    stroke_nums=stroke_nums.item(),  
                    delete_indices=DELETE_INDICES
                )

                stroke_nums = torch.tensor([new_stroke_num], device=stroke_nums.device)
