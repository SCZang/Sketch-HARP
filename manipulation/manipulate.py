...
TARGET_CATEGORY = 4   # to be edited sketch category
TARGET_SAMPLE_IDX = 2   # to be edited sketch idx
TARGET_STROKE_IDX = 2   # to be edited stroke idx
REPLACE_DICT={TARGET_STROKE_IDX: '/workspace/0529-17cate/sample/stroke_emb/sample_1_57/z_07.npy'}   # replace source dir
...
def visualize_strokes(padded_strokes, abs_start_positions, category, count, save_dir):
    padded_strokes = padded_strokes.detach().cpu().squeeze(0).numpy()        # [25, 32, 5]
    abs_start_positions = abs_start_positions.detach().cpu().squeeze(0).numpy()  # [25, 2]

    max_stroke_num = padded_strokes.shape[0]
    ...
    for i in range(max_stroke_num):
        ...
        start_pos = abs_start_positions[i]

        stroke = stroke[1:]  # 跳过 start_token
    ...
    save_path = os.path.join(save_dir, f"stroke_plot_{category}_{count}.png")
    plt.savefig(save_path, bbox_inches='tight', pad_inches=0.0)
    plt.close()

def replace_stroke(target_pad_path, target_pos_path,
                   source_pad_path, source_pos_path,
                   target_idx, source_idx):
    target_pad = np.load(target_pad_path)  # [1, 25, 32, 5]
    target_pos = np.load(target_pos_path)  # [1, 25, 2]
    source_pad = np.load(source_pad_path)  # [1, 25, 32, 5]
    source_pos = np.load(source_pos_path)  # [1, 25, 2]

    assert target_pad.shape == (1, 25, 32, 5), f"Unexpected pad shape: {target_pad.shape}"
    assert target_pos.shape == (1, 25, 2), f"Unexpected pos shape: {target_pos.shape}"

    new_padstroke = target_pad.copy()
    new_pos = target_pos.copy()

    new_padstroke[0, target_idx] = source_pad[0, source_idx]
    new_pos[0, target_idx] = source_pos[0, source_idx]

    return new_padstroke, new_pos

def convert_5d_to_3d(sketch_5d): 
    sketch_5d = np.asarray(sketch_5d)
    assert sketch_5d.shape[1] == 5, "输入必须是 (N, 5) 的 sketch-rnn 格式"


    dx_dy = sketch_5d[:, 0:2]
    pen = sketch_5d[:, 2:5]
    argmax_pen = np.argmax(pen, axis=1)

    state = np.where(argmax_pen == 0, 0, 1).reshape(-1, 1)

    sketch_3d = np.concatenate([dx_dy, state], axis=1)
    return sketch_3d
...
class Eval_Model:
    ...
    def sample(self, z, args, temperature=0.24, inject_stroke_emb_dir=None,replace_dict=None): 
        ...
        while inject_stroke_emb_dir and s in inject_stroke_emb_dir and "e" in inject_stroke_emb_dir[s]:
            ...
            if "p" in inject_entry:
                if isinstance(inject_entry["p"], str):
                    pred_pos = torch.tensor(np.load(inject_entry["p"])).cuda().float()
                else:
            ...
            injected_seq = np.load(inject_entry["s"]) if isinstance(inject_entry["s"], str) else inject_entry["s"]
            injected_seq = torch.tensor(injected_seq).cuda().float()
        ...
        for stroke_num in range(s, max_stroke_num):
            ...
            if replace_dict is not None and stroke_num in replace_dict:
                ...
                new_e = torch.tensor(new_e).reshape(1, -1).cuda().float()
                pred_stroke = new_e.unsqueeze(1)  # shape: [1, 1, emb_size]
    ...
    def eval(self, dataloader, category, target_index=None):
        ...
        for batch in dataloader:
            ...
            #np.save(f"/workspace/0529-17cate/enc_repalce/pad_stroke/stroke_{category}_{count}.npy",padded_strokes)
            #np.save(f"/workspace/0529-17cate/enc_repalce/pad_stroke/pos_{category}_{count}.npy",start_positions)

            '''
            target_pad_path = "/workspace/0529-17cate/enc_repalce/pad_stroke/stroke_1_3.npy"
            target_pos_path = "/workspace/0529-17cate/enc_repalce/pad_stroke/pos_1_3.npy"
            source_pad_path = "/workspace/0529-17cate/enc_repalce/pad_stroke/stroke_4_1.npy"
            source_pos_path = "/workspace/0529-17cate/enc_repalce/pad_stroke/pos_4_1.npy"

            target_idx = 2 
            source_idx = 3  

            new_padstroke, new_pos = replace_stroke(
                target_pad_path, target_pos_path,
                source_pad_path, source_pos_path,
                target_idx, source_idx
            )

            out_seqs_5d = padded_5d_to_strokes_5d(new_padstroke, new_pos)
            out_seqs_3d = convert_5d_to_3d(out_seqs_5d[0])
            np.save(f"/workspace/0529-17cate/enc_repalce/out_seqs_3d_{category}_{target_index}.npy", out_seqs_3d)
            print("saved")
            '''
            ...
            with torch.no_grad():
                ...
                out_padded_seqs, out_strokes, out_pos = self.sample(
                    seq_z, args,
                    replace_dict=REPLACE_DICT
                )
                visualize_strokes(out_padded_seqs, out_pos, category, count, save_dir='/workspace/0529-17cate/sample')
...
def sample(args):
    ...
    model.load(f'/workspace/0529-17cate/model/seq_enc_epoch_{args.epoch_load}.pth',
               f'/workspace/0529-17cate/model/stroke_enc_epoch_{args.epoch_load}.pth',
               f'/workspace/0529-17cate/model/stroke_dec_epoch_{args.epoch_load}.pth',
               f'/workspace/0529-17cate/model/seq_dec_epoch_{args.epoch_load}.pth',
               f'/workspace/0529-17cate/model/pos_dec_epoch_{args.epoch_load}.pth')

    for category in range(TARGET_CATEGORY,len(args.categories)):
        ...
        model.eval(test_dataloader, category,TARGET_SAMPLE_IDX)
        print("finish")
        break
