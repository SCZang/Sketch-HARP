...
TARGET_CATEGORY = 14   # to be edited sketch category
TARGET_SAMPLE_IDX = 119    # to be edited sketch idx
TARGET_STROKE_IDX = 1    # to be edited sketch stroke idx


SOURCE_EMBEDDING_PATH = "/workspace/0529-17cate/sample/stroke_emb/sample_14_117/z_00.npy"   # replace_target
...
def replace_stroke_embedding_single_sample(
    stroke_emb: torch.Tensor,         
    target_stroke_idx: int,             
    source_embedding_path: str ):       
    #在 sample 阶段，将单个样本中某个笔画的 embedding 替换为指定 .npy 文件中的 embedding

    assert stroke_emb.shape[0] == 1, "This function is only applicable for the case where batch_size = 1"
    assert os.path.exists(source_embedding_path), f"can't find{source_embedding_path}"

    new_embedding_np = np.load(source_embedding_path)  
    new_embedding = torch.tensor(new_embedding_np, dtype=stroke_emb.dtype, device=stroke_emb.device)

    stroke_emb[0, target_stroke_idx] = new_embedding
    return stroke_emb
    ...
    def eval(self, dataloader, category,target_index=None):
        ...
        for batch in dataloader:
            ...
            with torch.no_grad():
                ...
                stroke_emb = replace_stroke_embedding_single_sample(
                    stroke_emb,         
                    target_stroke_idx = TARGET_STROKE_IDX,            
                    source_embedding_path = SOURCE_EMBEDDING_PATH )       

                seq_z = self.stroke_encoder(stroke_emb, pos_emb, stroke_nums)
                out_padded_seqs, out_strokes, out_pos = self.sample(seq_z, args)
                visualize_strokes(out_padded_seqs, out_pos, category, count, save_dir='/workspace/0530-5cate-model/sample')
